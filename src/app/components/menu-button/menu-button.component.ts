...
export class MenuButtonComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscriptions.add(
      this.backgroundService.existsVaultState
    ...
    this.subscriptions.add(
      this.backgroundService.identitiesState
        .subscribe(identities => this.identities = identities)
    );
    this.backgroundService.getIdentities()
    .subscribe(identities => this.identities = identities);
  }
    ...
    if (address === '') {
      ...
      this.backgroundService.decodeBase58(address)
        .subscribe(bytes => {
          ...
          this.trustSvg = this.sanitizer.bypassSecurityTrustHtml(identicon.innerHTML);
        });
    }
  }

  changeAddresses(address: string) {
    this.backgroundService.changeAddress(address)
      .subscribe({
        next: () => this.zone.run(() => this.router.navigate(['/home'])),
        error: error => this.snackBar.open(error.toString(), '', {duration: 3000})
      });
  }
